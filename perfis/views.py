...
# --- IMPORT DO UTILITÁRIO REDIS (NOVO) ---
...
def enviar_solicitacao_amizade(request, receptor_id):
    ...
    if not ja_existe:
        ...
        # 🚀 NOTIFICAÇÃO VIA REDIS (Correção do erro AttributeError)
        # O canal deve ser o mesmo que o FastAPI está escutando
        publish_to_redis(
...
class MeuPerfilView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 🔥 ATIVIDADES
        context['activities'] = Atividade.objects.filter(
...
class VerPerfilView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 🔥 ADICIONA AS ATIVIDADES DO USUÁRIO VISITADO
        context['activities'] = Atividade.objects.filter(
